...
function emailFieldValidation(event) {
    ...
    if (!email.value.match(emailRegex)) {
        email.nextElementSibling.innerHTML ="Kindly enter a valid email";
        event.preventDefault();
        return false;
    }
    else {
...
function passwordValidation(event) {
    ...
    if(!password.value.match(pwdRegex)) {
        ...
        password.nextElementSibling.innerHTML = "Password length should be equal to or greater than 8";
        event.preventDefault();
        return false;
    }
    else {
...
function yearValidation(event) {
    ...
    if(!year.value.match(yearRegex)) {
        year.nextElementSibling.innerHTML = "Kindly enter a valid year";
        event.preventDefault();
        return false;
    }
    else {
